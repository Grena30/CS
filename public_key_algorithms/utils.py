...
public_exponent = random.randint(1, n_totient - 1)
...
def convert_str_to_hex(text_message):
    list_of_hex = []
...
def compute_rsa():
    message = input("\nEnter message: ")
    message_hex = convert_str_to_hex(message)
...
def compute_elgamal():
    ...
    beta_a = compute_beta_generator(prime_number, generator, private_key_elgamal)
    print("Beta generator:", beta_a)

    r, t = encrypt_elgamal(message_decimal, generator, beta_a, prime_number)
    ...
    decrypted_msg = decrypt_elgamal(t, r, private_key_elgamal, prime_number)
