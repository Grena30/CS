...
def sign_message(message_digest, public_key):
    return elgamal_encryption.encrypt_elgamal(message_digest, generator, public_key, prime_number)


def check_signature(message_digest, r):
    return elgamal_encryption.decrypt_elgamal(message_digest, r, private_key, prime_number)
...
def main():
    public_key = elgamal_encryption.generate_public_key(prime_number, generator, private_key)
    print("Public key:"
          "\n1. ", prime_number,
          "\n2. ", generator,
          "\n3. ", public_key)
    message = hash_to_decimal(hashed_message)
    r, signed_message = sign_message(message, public_key)
    obtained_message = check_signature(signed_message, r)

    print("\nHashed message:", hash_to_decimal(hashed_message))
    print("\nSigned message:", signed_message)
    print("Obtained message:", obtained_message)

    if message == obtained_message:
